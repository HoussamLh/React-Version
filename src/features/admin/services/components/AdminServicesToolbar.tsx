...
import type { ServiceStatus } from "../types/servicesCms.types";

type ServiceFilter = "all" | ServiceStatus;

interface AdminServicesToolbarProps {
  statusFilter: ServiceFilter;
  searchQuery: string;
  hasActiveFilters: boolean;
  filteredServicesCount: number;
  onSearchChange: (value: string) => void;
  onStatusFilterChange: (filter: ServiceFilter) => void;
  onResetFilters: () => void;
}

export const AdminServicesToolbar: React.FC<AdminServicesToolbarProps> = ({
  statusFilter,
  searchQuery,
  hasActiveFilters,
  ...
  return (
    <>
      ...
      <div style={styles.countRow}>
        <span style={styles.countBadge}>{filteredServicesCount}</span>

        <span style={styles.countText}>
          {filteredServicesCount === 1 ? "service" : "services"} shown
        </span>
      </div>
    </>
  );
