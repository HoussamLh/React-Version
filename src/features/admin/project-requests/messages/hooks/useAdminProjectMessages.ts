...
import {
  ...
  sendAdminProjectMessageWithMedia,
...
export const useAdminProjectMessages = (
  ...
  const loadMessages = useCallback(async () => {
    return getAdminProjectMessages(projectRequestId);
  }, [projectRequestId]);
  ...
  useEffect(() => {
    ...
    void markAdminProjectMessagesAsRead(projectRequestId)
      .then(() => onMessagesRead?.())
      .catch((readError) =>
        console.error(
          "Could not mark admin project messages as read:",
          readError,
        ),
      );
  }, [messages.length, projectRequestId, onMessagesRead]);
  ...
  useEffect(() => {
    ...
    const channel = client
      ...
      .on(
        ...
        (payload) => {
          const row = payload.new as ProjectMessageRow;

          const next: AdminProjectMessage = {
            id: row.id,
            projectRequestId: row.project_request_id,
      ...
      .on(
        ...
        (payload) => {
          const row = payload.new as Pick<ProjectMessageRow, "id" | "read_at">;
  ...
  const handleSend = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await sendAdminProjectMessageWithMedia({
        projectRequestId,
        message,
        images,
        files,
